    ...
    async (data: ChannelFormValues) => {
      ...
      try {
        if (isEditing && currentRow) {
          ...
          if (response.success) {
            const manualBalance =
              data.manual_balance && data.manual_balance > 0
                ? data.manual_balance
                : null
            const manualBalanceResponse = await updateChannelManualBalance(
              currentRow.id,
              manualBalance
            )
            if (!manualBalanceResponse.success) {
              throw new Error(
                manualBalanceResponse.message ||
                  t('Failed to update manual balance')
              )
            }
              ...
              >
                ...
                <CollapsibleContent className='mt-5 space-y-5'>
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              ...
                              <FormDescription>
                                {t(
                                  'Optional total available quota for this channel. Leave empty or 0 to use upstream balance.'
                                )}
                              </FormDescription>
                              <FormMessage />
                            </FormItem>
                  ...
                  <div className='bg-card space-y-4 rounded-xl border p-5'>
                    ...
                    <div className='space-y-3 rounded-lg border p-4'>
                      <SubHeading
                        title={t('Channel Call Limits')}
                        icon={<SlidersHorizontal className='h-3.5 w-3.5' />}
                      />
                      <div className='grid gap-4 sm:grid-cols-3'>
                        <FormField
                          control={form.control}
                          name='hourly_call_limit'
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel>{t('Hourly Call Limit')}</FormLabel>
                              <FormControl>
                                <Input
                                  type='number'
                                  min={0}
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel>{t('Daily Call Limit')}</FormLabel>
                              <FormControl>
                                <Input
                                  type='number'
                                  min={0}
