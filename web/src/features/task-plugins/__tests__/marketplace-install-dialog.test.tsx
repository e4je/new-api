...
test('first install shows source on demand and disables repeat submission until success', async () => {
  ...
  // CodeMirror token decoration depends on viewport geometry, which jsdom
  // does not provide. Verify the complete, read-only source review contract.
  const sourceViewer = await screen.findByRole('textbox')
  expect(sourceViewer).toHaveTextContent('// comment')
  expect(sourceViewer).toHaveTextContent('const plugin = "demo";')
  expect(sourceViewer).toHaveTextContent('function run() { return 42 }')
  expect(sourceViewer).toHaveAttribute('contenteditable', 'false')
