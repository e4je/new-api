...
func TestTaskPluginSourceAboveMySQLTextLimitRoundTrips(t *testing.T) {
	...
	// Recreate only this fixture's isolated table with the shipped legacy
	// column definitions, then exercise the real widening migration twice.
	type legacyTaskPlugin struct {
		model.TaskPlugin
		Source string `gorm:"type:text;not null"`
		Icon   string `gorm:"size:524288"`
	}
	table := database.NamingStrategy.TableName("TaskPlugin")
	require.NoError(t, database.Migrator().DropTable(&model.TaskPlugin{}))
	require.NoError(t, database.Table(table).AutoMigrate(&legacyTaskPlugin{}))
	legacy := legacyTaskPlugin{
		TaskPlugin: model.TaskPlugin{Key: "legacy", APIVersion: 1, Version: "1", SourceHash: "legacy-hash", Enabled: true},
		Source:     "preserved plugin source 中文",
		Icon:       "data:image/svg+xml;base64,legacy",
	}
	require.NoError(t, database.Table(table).Create(&legacy).Error)
	for range 2 {
		require.NoError(t, database.AutoMigrate(&model.TaskPlugin{}))
	}
	var preserved model.TaskPlugin
	require.NoError(t, database.First(&preserved, legacy.Id).Error)
	assert.Equal(t, legacy.Source, string(preserved.Source))
	assert.Equal(t, legacy.Icon, string(preserved.Icon))
	assert.Equal(t, legacy.SourceHash, preserved.SourceHash)
	assert.True(t, preserved.Enabled)
	duplicate := preserved
	duplicate.Id = 0
	assert.Error(t, database.Create(&duplicate).Error, "migration must preserve plugin key/version uniqueness")
	require.NoError(t, database.Delete(&preserved).Error)
	originalDB := model.DB
	model.DB = database
	t.Cleanup(func() { model.DB = originalDB })
